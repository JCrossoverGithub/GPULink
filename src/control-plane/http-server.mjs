  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/metrics") {
        ...
        return sendMetrics(response, database);
      }

      if (request.method === "GET" && url.pathname === "/v1/events") {
        if (!authorize(service, request, response, ["admin"])) return;
        return openEventStream(request, response, database, eventClients);
      }

      if (request.method === "POST" && url.pathname === "/v1/workers/register") {
        if (!authorize(service, request, response, ["worker"])) return;
        const worker = service.registerWorker(await readJson(request));
        broadcastNewEvents(database, eventClients);
        return sendJson(response, 200, { worker });
      }

      if (request.method === "GET" && url.pathname === "/v1/workers") {
        if (!authorize(service, request, response, ["admin"])) return;
        return sendJson(response, 200, { workers: database.listWorkers() });
      }

      const workerHeartbeat = matchPath(url.pathname, "/v1/workers/:id/heartbeat");
      if (request.method === "POST" && workerHeartbeat) {
        if (!authorize(service, request, response, ["worker"])) return;
        const worker = service.heartbeatWorker(workerHeartbeat.id, await readJson(request));
        broadcastNewEvents(database, eventClients);
        return sendJson(response, 200, { worker });
      }
      ...
      if (request.method === "POST" && workerDrain) {
        if (!authorize(service, request, response, ["admin"])) return;
        const worker = service.setWorkerDrain(workerDrain.id, await readJson(request));
        broadcastNewEvents(database, eventClients);
        return sendJson(response, 200, { worker });
      }
      ...
      if (request.method === "GET" && workerLeases) {
        if (!authorize(service, request, response, ["worker"])) return;
        if (!database.getWorker(workerLeases.id)) {
          return sendJson(response, 404, { error: { code: "not_found", message: "worker not found" } });
        }
        const jobs = database.listJobs({ workerId: workerLeases.id, limit: 100 })
      ...
      if (request.method === "GET" && url.pathname === "/v1/jobs") {
        ...
        return sendJson(response, 200, { jobs: database.listJobs({ status, limit }) });
      }

      const jobPath = matchPath(url.pathname, "/v1/jobs/:id");
      if (request.method === "GET" && jobPath) {
        if (!authorize(service, request, response, ["client"])) return;
        const job = database.getJob(jobPath.id);
        return job
      ...
      if (request.method === "POST" && jobStart) {
        if (!authorize(service, request, response, ["worker"])) return;
        const job = service.startJob(jobStart.id, await readJson(request));
        broadcastNewEvents(database, eventClients);
      ...
      if (request.method === "POST" && jobRenew) {
        if (!authorize(service, request, response, ["worker"])) return;
        const job = service.renewJob(jobRenew.id, await readJson(request));
        return sendJson(response, 200, { job });
      }
      ...
      if (request.method === "POST" && jobFinish) {
        if (!authorize(service, request, response, ["worker"])) return;
        const job = service.finishJob(jobFinish.id, await readJson(request));
        broadcastNewEvents(database, eventClients);
        return sendJson(response, 200, { job });
      }
      ...
      if (request.method === "POST" && jobCancel) {
        if (!authorize(service, request, response, ["client"])) return;
        const job = service.cancelJob(jobCancel.id);
        broadcastNewEvents(database, eventClients);
        return sendJson(response, 200, { job });
      }
...
function openEventStream(request, response, database, clients) {
  const lastEventId = Number(request.headers["last-event-id"] ?? 0);
  const client = { response, sequence: Number.isSafeInteger(lastEventId) ? lastEventId : 0 };
...
function broadcastNewEvents(database, clients) {
  for (const client of clients) flushClient(database, client);
}

function flushClient(database, client) {
  for (const event of database.listEventsAfter(client.sequence)) {
...
function sendMetrics(response, database) {
  const { workerCounts, jobCounts } = database.counts();
  const lines = [
    ...
    ...["queued", "leased", "running", "succeeded", "failed", "cancelled"].map((status) =>
